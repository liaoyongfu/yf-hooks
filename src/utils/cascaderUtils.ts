...
const getChildrenNodes = (data: CascadeOptions[], item: CascadeOptions, isChildren: isChildren) => {
    const findChildren = data.filter(
        (subItem: CascadeOptions) => subItem.value !== item.value && isChildren(item, subItem)
    );
    ...
    return findChildren.map((subItem: CascadeOptions) => {
        ...
        const subItemChildren = getChildrenNodes(data, subItem, isChildren);
...
export const transListToTree = ({ data = [], isChildren, isRoot }: transListToTreeProps) => {
    let dealData = [];

    dealData = data.filter(item => isRoot(item)).map(item => {
        const children = getChildrenNodes(data, item, isChildren);
        if (children) {
            return {
                ...item,
                children
 ...
 * @returns {[]}
 */
export const getSelectedOptions = (options: CascadeOptions[], value: string | number, valueKey: string) => {
    const { idMap, pidMap } = memoizedNodes(options, valueKey);

    return value
        ? [...pidMap[value].map((childId: string | number) => idMap[childId]), idMap[value]]
        : [];
};
